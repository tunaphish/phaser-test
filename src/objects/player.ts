...
export default class Player extends Phaser.GameObjects.Sprite {
  ...
  updateMovement(time, delta) {
    ...
    if (pointer.isDown && pointer.getDistance() > DEAD_ZONE) {
      let totalDistance = pointer.getDistanceX() + pointer.getDistanceY();
      velocityX = (SPEED * (pointer.x - pointer.downX)) / totalDistance;
      velocityY = (SPEED * (pointer.y - pointer.downY)) / totalDistance;

      let angle = utility.radiansToDegrees(pointer.getAngle());
